...
| RK-PTY-003 | `SET-PARTY` | Compare each member's status with their equipment and active effects. | Base and derived stats, level/progression, row, equipment, abilities, and status effects are readable. | Derived values agree with equipment/status rules and survive save/load. | M6.05, M7.12 / Gate 7 | Partial — M6 display/formula pass; save/load awaits M7.12 |
| RK-PTY-004 | `SET-PARTY` | Open Items and visit All, New, Recovery, Status, Battle, Material, Core, and Key tabs. | Each tab visibly contains only matching shared-inventory rows; empty and multi-page tabs render and scroll correctly. | Tab/filter/scroll navigation does not change quantities. | M6.06-M6.10 / Gate 6 | Pass — production-catalog fixture plus X11 screen 2026-08-12 |
...
| RK-PTY-010 | `SET-PARTY` | Open Equipment and visit every slot for a member with empty and populated slots. | Equipped item names and empty slots are distinct and readable. | Viewing equipment does not move shared-inventory items. | M6.15-M6.16 / Gate 6 | Pass — X11 Status plus equipment-domain fixture 2026-08-12 |
| RK-PTY-011 | `SET-PARTY`, compatible owned upgrade | Highlight the upgrade, inspect deltas, then equip it. | Every changed derived stat has a clear before/after preview and the equipped row updates. | New item equips atomically; the old item returns to shared inventory and totals update once. | M6.17-M6.18 / Gate 6 | Pass — Steel Axe swap fixture 2026-08-12 |
...
| RK-PTY-013 | `SET-PARTY`, member with learned, locked, field, and battle-only abilities | Open Spells for that member. | Only learned field-usable spells are selectable; locked and battle-only abilities are absent or clearly unavailable. | Viewing spells consumes no MP. | M6.20-M6.21 / Gate 6 | Pass — production class fixture 2026-08-12 |
| RK-PTY-014 | `SET-PARTY`, damaged member and field healing spell | Cast the spell on the damaged member. | Target and recovery feedback appear once. | MP decreases only after valid confirmation and HP caps at maximum. | M6.22 / Gate 6 | Pass — Elise Heal fixture 2026-08-12 |
...
| RK-PTY-016 | `SET-PARTY`, teleport ability and a mix of visited/unvisited/ineligible maps | Open the teleport destination picker and inspect it. | Only eligible visited destinations appear with accurate controls. | Opening/canceling the picker consumes no MP and changes no map. | M6.23 / Gate 6 | Pass — metadata/portal eligibility fixture 2026-08-12 |
