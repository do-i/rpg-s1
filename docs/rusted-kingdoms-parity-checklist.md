...
| ID | Setup / prerequisite | Exact player action | Expected visible / audible outcome | Expected state / persistence outcome | Target | Status |
| --- | --- | --- | --- | --- | --- | --- |
| RK-SAV-001 | `SET-NEW`, at a permitted field-save point | Open Save and inspect every slot. | Empty and valid slots are distinct; valid metadata includes enough identity/playtime/progress to choose safely. | Opening/canceling Save creates or changes no slot. | M7.07-M7.08 / Gate 7 | Pass — live field picker showed valid metadata and empty rows without writes on inspection |
| RK-SAV-002 | Empty slot from RK-SAV-001 | Select the empty slot and confirm save. | One success confirmation appears and the slot changes from empty to valid metadata. | A complete, versioned, loadable state is written atomically. | M7.01-M7.06, M7.08 / Gate 7 | Pass — live Slot 01 save refreshed metadata and produced a verified native v1 file |
| RK-SAV-003 | Existing valid slot plus changed current state | Choose that slot, cancel overwrite, reopen, and confirm overwrite. | Cancel and confirm are distinct; success metadata updates only after confirmation. | Cancel preserves old bytes/state; confirm atomically replaces the slot with the new complete state. | M7.05-M7.09 / Gate 7 | Pass — live confirmation/cancel preserved identical SHA-256; confirm/replace and temp cleanup pass focused tests |
