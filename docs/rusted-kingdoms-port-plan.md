...
| M0.17a | [ ] Decide the graceful Quit audio/exit lifecycle. | `T` | A short decision fixes the completion signal, fallback timeout, and test seam. |
| M0.17b | [ ] Implement and test graceful Quit. | `S` | Confirm audio starts, then exactly one exit is emitted after playback or a bounded failure fallback. |
...
| M1.05 | [ ] Route New Game to `NameEntry`. | `T` | Confirming New Game changes state once. |
| M1.06 | [ ] Add a reusable Back/Confirm/Up/Down action map. | `S` | Title input uses actions rather than raw keys. |
...
| M2.05 | [ ] Define the manifest service-sprite schema. | `T` | Apothecary, inn, item, weapon, armor, and item-box fields load. |
